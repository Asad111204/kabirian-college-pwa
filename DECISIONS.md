...
## ADR-171 - The office's own staff register: four statuses, and approved leave left out of the reckoning

**Status:** Accepted - 2026-09-08 - Phase 22

**Context.** The college asked for staff attendance taken by the office: **Present, Absent, Short Leave, Leave**. Until now the only register in the system was the class one, which a teacher fills in for their own section and subject and hands in. Staff attendance is a different thing wearing similar clothes, and copying the class design would have been the easy mistake.

**Decision.** One row per staff member per day (`staff_attendance`, unique on `(staff_id, date)`), and no sheet, no draft, no submit. The submit step exists so a teacher can hand a register in and the office can see it arrive; here the office *is* the one marking, so a mark is a fact the moment it is saved and a later change is a correction with a name on it. The audit log says which of the two happened: `staff_attendance.marked` the first time, `staff_attendance.corrected` when a mark that already existed moved.

The four statuses mean what the office means by them, and the percentage follows from that. PRESENT and SHORT_LEAVE both count as a day at work - somebody who left after the third period with permission was at work - and SHORT_LEAVE is kept as its own count so a pattern stays visible. ABSENT counts against. **Approved LEAVE comes out of the denominator entirely**: sanctioned leave is not a failure to attend, and a teacher who takes the leave the college granted must not watch their own figure fall for it. A month with nothing but leave in it has no percentage at all, and shows a dash rather than a nought that would read as a month of absence. The whole rule is `staff-attendance-policy.ts`, pure and tested from both sides.

Who appears on a day's register is worked out on the server from `joining_date` and `leaving_date`, never from the ids the browser sends. Every save rebuilds that roster and **refuses** an id that is not on it rather than skipping it quietly: a skipped id hides a disagreement between the screen and the server, and a refusal shows it. Backdating is ordinary office work - Monday morning, keying in the paper register - so any past day may be marked; the future may not, exactly as for students (ADR-078). Two permissions, `staff_attendance.view` and `staff_attendance.mark`, both on ADMIN only.

A staff member sees **their own** month on their own profile, read-only, with the same colour bands the students' figures use. It is their own record, so nothing in it is withheld; a correction is asked for rather than made there. The endpoint reads `ctx.staffId` and nothing else, so there is no id to tamper with.

**Alternatives.** *Reusing `attendance_sheets`* - the sheet is per section and subject and carries a teacher; staff attendance has none of those, and the columns would have been dead weight with a submit ceremony nobody wanted. *Counting LEAVE against the percentage, as the student setting can* - a student's guardian is told about every day missed; a staff member's approved leave was granted by the college itself. *Half-days as a fraction* - the college asked for short leave, which is a day at work with a note, not half a day.

**Consequences.** Through the production build: the office opened the register and marked two people, one present and one on short leave with a remark; a correction was recorded as a correction and its facts named the day and how many marks moved; tomorrow was refused with a sentence and the screen read as not editable; an id that was not a staff member gave a 404 and an invented status a 400; yesterday's paper register was keyed in; the month showed one present plus one leave as 100% and one present plus one absent as 50%; each teacher saw their own record and nothing of the other's; a teacher and a student were refused the register, the month and the marking, and were sent away from the page. Thirty-three checks, and the harness also caught an old accessibility defect on the way past: on a phone the account button had no accessible name at all, because the name is hidden below `sm` and an avatar without a photo is decorative. It now carries one. The migration was applied to Neon on 2026-09-08 (thirteen migrations, zero drift).

---

## ADR-172 · A complaint is between the student who wrote it and the office, and nobody else

**Status:** Accepted · 2026-09-09 · Phase 23

**Context.** The college asked for complaints: *a student submits an application; the admin reads and responds*. The obvious shape — a record with a body and a reply field — answers the letter of that and fails the first real use, because the office almost always needs to ask something back before it can act.

**Decision.** An application is one row that is **never edited once sent**. It is what the student said, on the record, and a student who could quietly rewrite it after the office had read it would be a different thing entirely. Everything after it is a message in `complaint_replies`, from one side or the other, so the exchange reads as a conversation and the original stays as it was written.

**Who can read one is the whole point.** The student who wrote it, and the office. Not another student, not a teacher — **not even a teacher holding both complaint permissions**, which the policy refuses explicitly and a test pins from both sides. A complaint may well be about a teacher, and half the reason for writing one is that it does not go round the college. A student asking after somebody else's application is told it does not exist rather than that it is not theirs, because the second answer confirms that it is.

**Nothing an application says reaches the audit log.** Four actions are recorded — written, replied to, state changed, withdrawn — with the category and the state and never a word of the wording. The audit log is read by every administrator with `audit.view`; the application is not.

**Four states, and who may move between them.** `SUBMITTED` when it arrives; `IN_REVIEW` once the office has answered, which happens by itself when the office replies rather than needing a second button; `RESOLVED` when the office is done, optionally with a closing message that goes to the student as the answer; `WITHDRAWN` when the student takes it back. The office may pick a resolved application back up — an answer that turned out to be wrong should be corrected on the same application rather than buried — but it **never withdraws on a student's behalf**, and it never undoes a withdrawal. Withdrawing is not deleting: the application is kept, marked.

**Whose move it is** is stored on the row (`awaiting_office`) rather than worked out from the last message, so the office's "everything still on my desk" is one indexed query and the dashboard tile is a count rather than a scan. One pure function decides what to store, so the flag cannot drift from the story the messages tell.

Two smaller decisions. A student may have **five applications open at once**, refused with a sentence that says so, because an office that cannot see the wood for the trees answers nothing. And the office's replies reach a student as **"The college office"** while the office itself sees which person wrote them: the college answers as an institution, and accountability stays inside it.

**Alternatives.** *A single response field* — the college would have received a second complaint every time it needed to ask a question. *Attachments* — a photograph of a voucher would help, and was not asked for; the documents service is tied to owner types and would need widening, so it is left for a request. *Letting teachers see complaints about their own subject* — the exact thing a student would be afraid of. *Anonymous complaints* — not asked for, and unanswerable.

**Consequences.** Through the production build: a student wrote one and it came back waiting on the office; four words were refused, and so was a category the college does not have; neither the office nor a teacher can write one; the office and the writer could read it, another student got a 404 with not one word of it in the reply, and a teacher got a 403; the office answered and the application moved itself to "being looked at" and off the "waiting on us" list; the student saw the reply as the college's and the office saw the clerk's name; a student could not mark their own application resolved, the office could not withdraw one or act when it was already in that state; after resolving, neither side could add anything and the reason said so; the office picked it back up; the second student withdrew their own and the office could not undo it; every audit entry carried the category and none of the wording; the sixth open application was refused. Sixty-one checks. The migration was applied to Neon on 2026-09-09 (fourteen migrations, zero drift).

**One thing fixed on the way past.** The permission table said a teacher holds `staff_attendance.view` and `staff_attendance.mark` — added by mistake in Phase 22. The service refused them anyway, since staff attendance asserts the office before it checks anything else, so no teacher could ever mark a register; but the table was wrong and the account screens said so. A duplicated pair of homework entries went with it.

---

## ADR-173 · One account, two portals: the portal you are in is the role you are

**Status:** Accepted · 2026-09-09 · Phase 24

**Context.** The college asked for a member of staff who is also an administrator to have **one account** rather than two, and to move between the staff portal and the office portal without signing out. Students stay single-role; this is deliberately the narrow version, confirmed on 2026-09-07.

The danger in it is not the feature, it is the blast radius. `ctx.role` is read in eighty-five places across fourteen services, and every one of them means something by it: the office may read any section's register, a teacher only their own; the office may correct published marks, a teacher may not. Any design that made `role` ambiguous would have needed all eighty-five re-examined, and would have left a permanent trap for the eighty-sixth.

**Decision.** An account has a **set** of portals it may use and exactly **one** it is working in, and the one it is working in is what `ctx.role` has always meant. A principal in the staff portal *is* a teacher: teacher's permissions, teacher's scope, teacher's screens. The same person in the office portal is the office. Not one of those eighty-five call sites had to change, and the eighty-sixth will be right by default.

The set is the security boundary; the switch is a convenience. Somebody who holds both portals can always move to the other one, so refusing them in the portal they are not currently in protects nothing — it is only about which hat they are wearing. That is why a bookmark into the office does not fail for a principal: it offers the switch. And it is why granting *less* than the account could reach is safe, while the reverse would not be.

**Where each piece lives.** `users.admin_access` is the set, and a CHECK constraint keeps it to staff accounts, because an administrator already has the office and a student never does. `sessions.active_role` is the hat, kept on the session row rather than in a cookie so the browser has nothing to edit. It is **resolved against the set on every request**, so the moment office access is taken away, a session sitting in the office portal falls back to the staff one — without anybody having to remember to end that session, and without signing the person out of work they are in the middle of.

Switching is a POST, never a side effect of loading a page, and it is audited: `auth.portal_switched` with where they came from and where they went, so the log reads as one person rather than two mysterious ones. Granting and revoking office access are audited as their own actions.

**Two safety rules kept deliberately conservative.** Nobody changes their own office access, even from the office — in practice a staff account cannot, since it needs `users.manage`, but "in practice" is how escalation happens. And a staff-admin is **not** counted as an administrator by the last-administrator rules that stop the college locking itself out: those refuse more often than strictly necessary, which is the right direction for a rule whose failure mode is nobody being able to get in.

**Alternatives.** *Two accounts* — what the college has now and asked to stop: two passwords, two audit trails, and no way to tell that Miss Sara the teacher and Miss Sara the clerk are one person. *A role array on the user, with permissions unioned* — the trap described above: every `ctx.role === 'ADMIN'` becomes ambiguous, a teacher's own screens start showing the whole college, and eighty-five decisions have to be re-made at once. *Deciding the portal from the URL* — impossible for the shared API routes, which are called from both portals and carry no hint of which page they were called from.

**Consequences.** Through the production build, with a fixture teacher who is also an administrator: before switching they are refused the office's register, the accounts list and students' applications exactly like any teacher, and their own staff pages work as before; a link into the office offers the switch while a single-portal teacher is still sent home; the switch page asks rather than acting; a teacher without access, a student and an administrator are all refused the switch; after switching, the same person opens the register, the accounts list, the applications and the office dashboard, and is offered the way back, while a portal they do not hold is still refused; their own staff record stays theirs in both. Office access cannot be given to an administrator, to a student, to yourself, or by a teacher, and giving it to somebody who has it is a conflict rather than a silent no-op; granting it lets an existing session switch straight away, and revoking it puts that session back in the staff portal on the next request. Forty checks, and every one of the other 530 still passes — which is the real result, since the fixture teacher holds the office throughout them.
