...
**Consequences.** Through the production build: the office opened the register and marked two people, one present and one on short leave with a remark; a correction was recorded as a correction and its facts named the day and how many marks moved; tomorrow was refused with a sentence and the screen read as not editable; an id that was not a staff member gave a 404 and an invented status a 400; yesterday's paper register was keyed in; the month showed one present plus one leave as 100% and one present plus one absent as 50%; each teacher saw their own record and nothing of the other's; a teacher and a student were refused the register, the month and the marking, and were sent away from the page. Thirty-three checks, and the harness also caught an old accessibility defect on the way past: on a phone the account button had no accessible name at all, because the name is hidden below `sm` and an avatar without a photo is decorative. It now carries one.
