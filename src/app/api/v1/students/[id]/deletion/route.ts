...
import { deleteStudent, getStudentDeletionReport } from '@/server/services/deletion.service'
...
/** GET /api/v1/students/:id/deletion — whether they can be erased, and what stands in the way. */
export const GET = withAuth(async ({ ctx, params }) => jsonOk(await getStudentDeletionReport(ctx, idOf(params.id))))

/** DELETE /api/v1/students/:id/deletion — erase them, for good. */
export const DELETE = withAuth(async ({ request, ctx, params }) => {
  ...
  return jsonOk(
    await deleteStudent(ctx, idOf(params.id), parsed.data.confirm, { ipAddress: clientIp(request), userAgent: request.headers.get('user-agent') }),
  )
})
