...
How the Kabirian College Management System goes live and stays live. Written for the person who runs it, not for a hosting expert; every step says what it is for.
...
1. Create a project on [neon.tech](https://neon.tech) (free tier) in a region near Pakistan (Singapore is the closest). Neon keeps **point-in-time history** — on the free tier, the last 6 hours; on paid plans, days. That is the first line of defence against a mistake; the college's own backups (section 6) are the second.
...
3. Apply the migrations from your computer, never from the host:
   ...
   npm run db:migrate
   ```
4. Seed the reference data (permissions, designations, document types) and the college's structure, then create the first administrator:
   ```
...
5. **Least privilege** (optional but recommended before real data): run `scripts/db-least-privilege.sql` in Neon's SQL editor as the owner role, with a fresh password in place of the placeholder. Then use `kabirian_app` in the pooled `DATABASE_URL` on the host. The owner role stays on your computer for migrations, backups and restores.
...
| `APP_COLLEGE_NAME` | The college's name as it should appear |
...
| `DATABASE_URL` | Neon's pooled string (`kabirian_app` after section 1.5) |
...
4. **Domain**: Settings → Domains → add the college's domain and follow the DNS instructions (a CNAME to Vercel). HTTPS is automatic. Then set `APP_URL` to the `https://` domain and redeploy.
...
docker run -d --name kabirian -p 3000:3000 --env-file /path/to/production.env --restart unless-stopped kabirian-college
```
...
Neon's history covers "undo the last few hours". The college's own backup covers "the account was lost", "the region was down" and "we want a copy we hold":
...
- Run the export **weekly** on the administrator's computer with `DATABASE_URL` set to the production string, and copy the folder to a drive the college controls (an external disk, or the college's Google Drive by hand). It contains everything — names, CNICs, password hashes — so keep it where only the administration can reach it.
...
- **Errors**: the app logs one JSON line per event with passwords, tokens and national IDs redacted. On Vercel: project → **Logs**; in Docker: `docker logs kabirian`. Look for `"level":"error"`.
...
- [ ] Least-privilege role in place; host uses `kabirian_app`
...
- [ ] Google Drive connected as the college's account; folders created; one test upload and download
