...
check('admin’s real upload passes validation and reaches storage (off here → 503)', r.status === 503, `${r.status} ${r.error?.message?.slice(0, 60)}`)
...
check('opens their section’s notice, with its attachment listed', r.status === 200 && r.data.attachments.length === 1)
...
check('their notice’s attachment passes the access check (storage is off here → 503)', r.status === 503, String(r.status))
...
check('the staff event’s attachment passes the access check for staff (→ 503, storage off)', r.status === 503, String(r.status))
