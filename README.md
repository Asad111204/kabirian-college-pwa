...
| **Correct their details** | **Edit details** on the profile opens the same boxes the admission form uses, filled in. Every detail the college records can be corrected, and each correction is in the audit log with who made it. The Student ID never changes, and where they sit is moved by Transfer or Promote so the history is kept. |
...
| **Correct their details** | **Edit details** on the profile opens the same boxes the "add staff" form uses, filled in — designation, department, qualification, salary, contact, everything. Each correction is in the audit log. The Staff ID never changes, and whether they are still employed is changed by the status action, which asks for a reason. |
